...
void testSequenceSubSeq() {
    int arr[] = {5, 10, 15, 20};
    SmartListSequence<int> sequence(arr, 4);
    UniquePtr<Sequence<int>> subSequence = sequence.getSubSequence(1, 2);
...
void testSequenceConcat() {
    ...
    UniquePtr<Sequence<int>> uPtr2(new SmartListSequence<int>(std::move(sequence2)));
    sequence1.concat(std::move(uPtr2));
